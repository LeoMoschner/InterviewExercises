...
/**
 * We accidentally dropped the database where we store the current billing status for our advertisers.
 * Fortunately, we still have the logs for all the transactions they did, and we can use this to recreate the dropped data.
 ...
 * You should create a class called BillingStatus, which will represent an account state.
 * Each financial transaction represents a modification to a BillingStatus.
 * A BillingStatus should be able to ingest new transactions that are generated in our own systems.
 ...
 * <p>
 * Each transaction can have multiple monetary columns.
 * Upon processing a transaction, the values in the monetary columns should be added to the current value in the BillingStatus.
 * <p>
 * Expected Output
