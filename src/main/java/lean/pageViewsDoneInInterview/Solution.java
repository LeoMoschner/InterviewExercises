...
/**
 * Find total numbers of pv for android over the last 7 days
 */

class Solution {
    public static void main(String[] args) {
        System.out.println("Hello, World");
        Map<String, DateEntry> data = InputClassJava.generateDataInMapFormat();
        int days = 2000;
        getLastWeekAndroidPV(data, days);
    }

    public static long getLastWeekAndroidPV(Map<String, DateEntry> data, int days){
        long result = 0;
        LocalDate validDate = LocalDate.now().minusDays(days + 1);
        for (Map.Entry<String, DateEntry> entry: data.entrySet()){
            try{
                LocalDate dateToCheck = LocalDate.parse(entry.getKey());
                if (dateToCheck.isAfter(validDate) && entry.getValue().android != null){
                    result += entry.getValue().android.pageViews;
                }
            }catch (DateTimeException e){
                System.out.println("Error: not valid input" + e);
            }

        }
        System.out.println("Android page views of the last " + days + " days: " + result);
        return result;
    }
}
...
class InputClassJava {
    ...
    static String inputString = "2019-11-26\n" +
            "2023-02-08,android,1336061,375491,iOS,985014,418290,mobileWeb,336195,198403,newReddit,267522,140548,oldReddit,957462,189477\n" +
            ...
            "2023-02-01,android,1291546,280199,iOS,575184,257443,mobileWeb,191464,116411,newReddit,225093,118371,oldReddit,760191,145437\n" +
