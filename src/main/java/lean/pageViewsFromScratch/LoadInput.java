...
public class LoadInput {
    static String inputString = "2023-01-26\n" +
            "2023-02-02,android,1336061,375491,iOS,985014,418290,mobileWeb,336195,198403,newReddit,267522,140548,oldReddit,957462,189477\n" +
            "2023-02-03,android,1153728,298063,iOS,755030,311642,mobileWeb,219457,130288,newReddit,221081,111902,oldReddit,723509,132801\n" +
    ...
    public static Map<String, PlatformDTO> getFormattedMap(){
        Map<String, PlatformDTO> result = new HashMap<>();
        ...
        for (String line: inputLine){
            ...
            int pageViews = 0;
            int uniques = 0;
            PlatformDTO platformLoadAux = new PlatformDTO();
            for (int i = 1; i < splittedLine.length; i++){
                platformName = splittedLine[i];
                pageViews = Integer.valueOf(splittedLine[++i]);
                uniques = Integer.valueOf(splittedLine[++i]);
                ViewsDTO viewsLoadAux = new ViewsDTO(pageViews, uniques);
                switch (platformName){
                    case "android": platformLoadAux.setAndroid(viewsLoadAux);break;
                    case "iOS":platformLoadAux.setIos(viewsLoadAux);break;
                    case "mobileWeb":platformLoadAux.setMobileWeb(viewsLoadAux);break;
                    case "newReddit":platformLoadAux.setNewReddit(viewsLoadAux);break;
                    case "oldReddit":platformLoadAux.setOldReddit(viewsLoadAux);break;
                    default:
                        System.out.println("ERROR: Wrong platform name");
                }
            }
            result.put(date, platformLoadAux);
        }
        return result;
    }
}
