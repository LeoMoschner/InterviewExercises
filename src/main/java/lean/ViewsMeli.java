...
/**
 * Después me pasó una archivo con clases que producían el json que me dio primero
 * y me pidió que devuelva el numero de page views por plataforma de los últimos 7 días.
 * */
public class ViewsMeli {
    public static void main(String[] args) {
        System.out.println(getLastPageViews());
    }
    public static long getLastPageViews(){
        Map<String, DateEntry> dataMap = InputClassJava.generateDataInMapFormat();
        int lastDays = 7;
        LocalDate validDate = LocalDate.now().minusDays(lastDays + 1);
        long pageViews = 0;
        for (Map.Entry<String, DateEntry> firstEntry: dataMap.entrySet()){
                LocalDate dateToCheck = LocalDate.parse(firstEntry.getKey());
                if (dateToCheck.isAfter(validDate)){
                    pageViews += firstEntry.getValue().android.pageViews;
                    pageViews += firstEntry.getValue().ios.pageViews;
                    pageViews += firstEntry.getValue().mobile_web.pageViews;
                    pageViews += firstEntry.getValue().new_reddit.pageViews;
                    pageViews += firstEntry.getValue().old_reddit.pageViews;
                }
        }
        return pageViews;
    }
...
class InputClassJava {
    public static void main(String[] args) {
        //Map<String, Map<String, Map<String, Integer>>> asd = Map.of("2020-01-20", Map.of("android",));
    ...
    static String inputString = "2023-01-26\n" +
            "2023-01-21,android,1336061,375491,iOS,985014,418290,mobileWeb,336195,198403,newReddit,267522,140548,oldReddit,957462,189477\n" +
            "2023-01-24,android,1153728,298063,iOS,755030,311642,mobileWeb,219457,130288,newReddit,221081,111902,oldReddit,723509,132801\n" +
            "2023-01-26,android,1291546,280199,iOS,575184,257443,mobileWeb,191464,116411,newReddit,225093,118371,oldReddit,760191,145437\n" +
            "2023-01-25,android,1613490,331628,iOS,626490,288464,mobileWeb,193197,111844,newReddit,300378,175446,oldReddit,830788,164052\n" +
            "2023-01-01,android,1075637,310187,iOS,868377,375826,mobileWeb,223399,122215,newReddit,356930,208573,oldReddit,983578,197101\n" +
            "2023-01-02,android,1211191,355036,iOS,1005068,423271,mobileWeb,308485,161385,newReddit,426082,235286,oldReddit,1044983,224812\n" +
            "2023-01-03,android,1142720,326486,iOS,1002917,408472,mobileWeb,333752,176264,newReddit,381209,202905,oldReddit,1071201,218746\n" +
            "2023-01-04,android,896853,292943,iOS,883463,372019,mobileWeb,309958,165411,newReddit,272320,153546,oldReddit,813187,176295\n" +
            "2023-01-05,android,859666,250634,iOS,776614,301808,mobileWeb,252932,136008,newReddit,203651,104984,oldReddit,698273,133476\n" +
            "2023-01-06,android,893927,268507,iOS,721442,301548,mobileWeb,272139,172320,newReddit,273291,148183,oldReddit,830540,163286\n" +
            "2023-01-07,android,720578,228043,iOS,610685,273632,mobileWeb,233233,145642,newReddit,246216,131454,oldReddit,809409,164890\n" +
            "2023-01-08,android,866966,257861,iOS,696012,295133,mobileWeb,146846,91303,newReddit,299146,153051,oldReddit,746445,165837\n" +
            "2023-01-09,android,917009,297440,iOS,771828,369154,mobileWeb,281645,176258,newReddit,321270,182655,oldReddit,979610,217932\n" +
            "2023-01-10,android,1017050,297403,iOS,786895,340634,mobileWeb,303616,191342,newReddit,308475,169901,oldReddit,936982,189128\n" +
            "2023-01-11,android,1155272,344532,iOS,961367,414086,mobileWeb,316443,180163,newReddit,291638,157320,oldReddit,992157,209716\n" +
            "2023-01-12,android,934881,277578,iOS,753657,330896,mobileWeb,229383,131696,newReddit,242391,127208,oldReddit,759403,147741\n" +
            "2023-01-13,android,1225972,353731,iOS,1149590,458661,mobileWeb,309976,172001,newReddit,414602,225001,oldReddit,1155450,217974\n" +
            "2023-01-14,android,1149410,395503,iOS,1261771,509686,mobileWeb,301117,182553,newReddit,650360,365394,oldReddit,1049098,248047\n" +
            "2023-01-15,android,1189013,361782,iOS,1222918,491474,mobileWeb,340257,178917,newReddit,377485,212397,oldReddit,1117135,222187\n" +
